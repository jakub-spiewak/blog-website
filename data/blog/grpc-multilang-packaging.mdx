...
And in those folders we would generate code into them.
...
Finally, we can create our script that would make all our jobs for us, from project setup to publish generated code.
Firstly, we'll create script file, lets name it `run.sh` and place it in `./script` folder.
...
Those constants are paths to our previous created directories and files.
...
Later we'd use that function and use it on every generated `.js` file.
